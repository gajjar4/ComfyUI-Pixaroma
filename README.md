...
- **NEW: Longest Side Pixaroma.** Most resizing comes down to one sentence: make this picture 1216 on its longest side. This node is that sentence and nothing else, which is why it stays two rows tall. Click a size and the longer edge becomes exactly that, with the other edge following so nothing is squashed. It works the same for a tall photo or a wide one, so you never have to work out whether you mean width or height. The shape chips underneath crop first if you want them to: **keep** leaves your picture's shape alone, or pick 1:1, 16:9, 9:16 or 2:3 and the biggest piece of that shape is taken out of your picture, so nothing stretches and no empty bars appear. A small button rounds both sides to 8, 16, 32 or 64, which is what most models want. The node shows the size it is going to send **before you run anything**, by reading the picture coming in. Behind the gear: which sizes and shapes appear on the rows (type any size you like), which part of the picture a crop is taken from, whether small pictures may grow, the resample quality and the node's colour. Reach for Image Resize Pixaroma instead when you need to set a width **and** a height.

### **August 4, 2026 · v1.4.85–v1.4.90**
- **Your own LoRA trigger words are now kept with the LoRA.** If a LoRA has no trigger words of its own, you can type your own in its info panel. Those words were only remembered on that one row, so swapping the row to another LoRA and back lost them, and the same LoRA used in another node or another workflow never had them. They now belong to the LoRA itself: type a word once and it is waiting in that LoRA's panel wherever you use it. Words you have already typed are kept, they move across the first time you open the panel. They are saved on your own computer alongside your other Pixaroma settings, so an update never wipes them and nothing is written into your models folder. As before, tapping a word is what sends it to the triggers output, so being remembered means it is offered again, not switched on for you.
- **Sizes can mark your recommended sizes with a star.** A workflow list often holds every size that works while only some suit the model you are actually running. Click the star beside a size in the settings and that size gets a star on the node, so the good ones stand out at a glance. Stars follow the size itself, so reordering your list, flipping between portrait and landscape, or removing a size and adding it back all keep them. A star is a reminder only and changes nothing about what the node sends out.
- **Switch and Mute Switch no longer break when you change the node style.** ComfyUI has a setting that swaps between the classic node look and the newer one. Changing it without refreshing the page left these two nodes wrong: switching one way emptied the node completely, and switching the other way drew the rows twice, one set on top of the other. Refreshing the page always fixed it, which is why it looked like a problem with your ComfyUI version. Both now rebuild themselves the moment you change the setting, with your rows, names and chosen input all intact.
- **It is easier to find where your own fonts go.** People adding fonts for Text Overlay, Text Watermark and Image Composer text had no way of knowing where to put them. The font list now says so at the bottom: put `.ttf` or `.otf` files in `ComfyUI/models/fonts` and press the refresh button. There is also a new "Add your own fonts" page in the Help window covering the whole thing, including adding a full family so bold and italic work properly, and keeping fonts on another drive.
- **Pixaroma now works properly on online ComfyUI services.** If you use ComfyUI through a website rather than on your own PC, a lot of the pack was quietly broken there: the Save Mp4 player was black and would not play, Note icons showed as empty squares, fonts and sounds were missing, and various pickers and panels did nothing. The files themselves were always made correctly. The problem was your browser asking for them at an address the service does not hand to ComfyUI, so it was turned away. Around 270 addresses across the pack were built as if ComfyUI owned the whole website, which is true on your own PC and not on a shared service. They are now built the way those services expect, including the sign-in these sites attach to every request, which is what the well behaved node packs already do. Icons, fonts, sounds and 3D models are also fetched a different way now, because those services hand pictures and sound files to their own file server instead of to ComfyUI. Nothing changes when you run ComfyUI on your own computer.
- **Our full screen editors no longer hold up the rest of ComfyUI.** While a Pixaroma editor is open, Paint, Note, 3D Builder, Image Composer, Image Crop and the others, pressing Ctrl+Z is kept inside that editor so it cannot undo your workflow behind your back. That part is unchanged. The old way of doing it also switched off workflow loading for the whole page while an editor was open, so nothing at all could open a workflow at that moment, including nodes made by other creators. This matters most on the online ComfyUI services, where every add-on shares one page. It now uses a switch ComfyUI provides for exactly this, so Ctrl+Z is still contained and everything else is left alone. Tested on every editor: undo stays inside, and workflows load normally again while one is open.
...
- **It shows you the size it will send**, like 896x1312, right on the node, and it updates as you change the step, flip the orientation or edit the numbers. If a size is coming in from a wire it says so instead of guessing.
- **Your existing Portrait Landscape nodes are unaffected**: they open with the orientation you saved and the rounding switched off.
- **NEW: Duration Pixaroma.** Say how long a video should be in seconds and it works out the frame count your model actually wants. It replaces the two nodes people usually wire up for this: one holding the number of seconds, and one doing maths on it. Pick your model in the settings and the numbers fill in, or type them yourself for a model that is not listed.
- **You decide which lengths it offers.** Give it a short list like 3, 5, 10 and 15 and you get buttons to click. Give it a smallest and largest instead and you get a slider. Two of these on one canvas can be set up completely differently, so a workflow only ever offers lengths that make sense for it.
- **It tells you what it will send before you run**, including the true length, because rounding to your model's pattern usually shifts it a little: 5 seconds at 24 frames per second becomes 124 frames, which is really 5.17 seconds. The settings list every allowed length and what each one becomes, so a wrong setting shows up before you spend a render, not after.
- **Two outputs**: the frame count for your video node, and the real length in seconds for anything that has to line up with the picture, such as audio.
- **Prefer to write the maths yourself?** Choose Custom formula and type it. It understands the same functions as ComfyUI's own Math Expression node, so you can paste one straight across.
- **Fixed: Control Panel and Dropdown could not connect to some newer ComfyUI nodes**, including its own Math Expression. The wire appeared and then vanished a moment later, with a message blaming the wrong thing. They now connect properly, and still refuse the inputs they genuinely cannot drive.
- **Connection sparkles now light up on those inputs too**, so the orange markers show every place a wire can actually land.
...
- **Prompt: put your tag categories in any order** by dragging or the ⋯ menu, and drag the divider to widen the list.
- **Prompt: colours now show where each piece came from** - tag, category or list - in the box and in the expanded preview.
- **Prompt: a half-typed tag gets a spellcheck-style underline** instead of glowing red at you the whole time.
- Plus fixes: dragging a workflow or a category onto a text box could rename a file or edit a tag; renaming a category to change only its capitals did nothing; Inpaint Crop failing on newer NumPy.
