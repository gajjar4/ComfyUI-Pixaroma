...
- **The frames output is already snapped to the pattern H3 accepts,** so the video comes out exactly as long as the prompt was written for. Getting those two out of step is the easiest way to waste a render. It also works for Wan, Hunyuan and LTX, or you can turn the snapping off.
- **It runs entirely on your own machine.** No account, no key, nothing sent anywhere. You need one vision language model in your `models/text_encoders` folder, about 5 to 10 GB depending which you pick, and the node finds a suitable one by itself if the one in the settings is not there. The Help page names the exact model everything was tested against and links to it.
- **Every word of the wording it follows is yours to edit,** in the settings, and your edits are kept outside the plugin folder so updating Pixaroma never overwrites them. Reset puts the shipped version back. You can also switch our length instructions off completely and use your own wording, for any video model you like.
- **Talking prompts now work at 5 seconds.** The five second setting used to drop the spoken line every single time, so you had to use eight seconds or more if anyone in your clip speaks. It writes the line most of the time now. Eight and ten seconds are still the surest, and the node quietly marks the five second button when it notices speech in your idea.
- **A Free VRAM switch** beside Copy. Leave it off while you are only writing prompts so generating again is instant, and turn it on when this node shares a workflow with your video model: the language model is handed back the moment the prompt is written. On one machine that freed about 17 GB.
- **Prompt Reader Pixaroma can pull your idea back out** of a picture made with this node, so you can find the words you typed weeks later.

### **August 10, 2026 · v1.4.103**
- **Fixed: Save Image showed the old picture after you deleted a saved file.** If you deleted the newest image from your folder, the counter correctly reused that number for the next run, the new image was written correctly, but the node kept showing the deleted one. The file on disk was always right; only the picture on the node was wrong, which made it look like the save had failed. Your browser was reusing the picture it already had, because the address was identical for the same filename. Thank you to the person who reported it with the exact steps, which is what made it quick to reproduce.
...
- **Fixed: the wrong kind of sound connection stopped the whole run and left an empty file behind.** If anything other than real audio reached the Save Mp4 sound input, which is easy to do through nodes that pass any type of connection along, the run stopped with an error and left a 0 byte video sitting in your output folder every time. It now simply makes the video without sound and tells you why in the log.
- **Fixed: frames with transparency produced a broken video and said nothing at all.** Save Mp4 expects ordinary colour frames. If frames carrying transparency arrived instead, it wrote a video that looked like a normal file but had scrambled picture in it, with no error from anywhere. It now stops with a clear message telling you to convert to normal colour first, and no stray file is left behind.
- **New: Save Image can now save WebP, and it is probably the format you want.** Same picture, roughly a fifth the size of a PNG, transparency kept, and unlike JPG you can still drag the file back into ComfyUI to reload the whole workflow. One 1024 x 1024 render went from about 1.1 MB as a PNG to under 200 KB. There is a WebP lossless switch in the settings for when a file has to be pixel perfect, which still comes out smaller than a PNG.
- **New: a settings gear on the Save Image node**, next to the fold triangle, so you no longer have to know to right-click. The panel also follows the node now when you zoom or pan around the canvas.
- **New: hide the buttons you never use.** In the Save Image settings there is a Buttons on the node row where you can switch off Open, Copy, Folder, or any of the three formats. Useful if the Folder button does nothing helpful on your system, or you only ever save as PNG and WebP and want JPG out of the way.
- **New: keep your folder structure when processing a whole folder.** Turn on Keep folder structure in the name in Load Images from Folder, and Keep folders from the wired name in Save Image, and a folder full of subfolders comes back arranged exactly the same way in your save folder. Both are off by default, so nothing changes unless you ask for it.
- **New: a + Input folder chip** that puts each picture in a folder named after whatever is wired into the name input, in one click.
- **Better help for Save Image.** The help page now has a table of worked examples showing what to type and what actually lands on disk, a plain English explanation of what the name input does (the short version: the slash is what makes a folder), and a size comparison of the three formats. Every example was produced by running the node, not written from memory.
- **Fixed: a saved picture carried the previous run's preview.** Drag a saved image back into ComfyUI and the Save Image node showed a different picture, only correcting itself when you pressed Run. ComfyUI freezes the workflow into the picture at the moment you hit Run, which is before that picture exists, so the node was remembering the run before. It now starts empty until you run, which is what ComfyUI's own Save Image does.
- **Fixed: the Will save as line could promise a file name you would not get.** A few patterns, such as one starting with a slash or containing a stray dot at the end, were shown differently from what was really written. That line is the one thing the node asks you to trust, so it now matches the saved file exactly.
- **Fixed: Load Image Mini showed the wrong picture after you switched workflow tabs.** The file name in the picker was right, but the picture underneath it, and the size shown on the INPUT card, were the ones from before you changed it. This is the other half of the problem fixed on August 7: the name was being remembered from then on, but ComfyUI keeps its own copy of the picture for each node, and ours was not being kept up to date. So when you came back to the workflow, ComfyUI loaded two pictures at once, the old one and the new one, and whichever finished first was the one you were left looking at. That is why it happened some times and not others. It mattered beyond just looking wrong, because that picture is also what the Mask Editor opens and what the INPUT size is read from. The same fix covers Load Image as well. Thank you to the person who sent a screen recording, which is what made it possible to catch a three millisecond race.
...
- **NEW: Load Audio Pixaroma.** Pick a sound file and take exactly the piece you want, without cutting it up in another program first. The file is drawn as a waveform on the node: drag either orange edge to trim, drag the middle to slide, click anywhere to place the play cursor, and press play to hear your selection before you spend a render on it. Wire the seconds output of **Duration Pixaroma** into it and the window becomes exactly as long as the video you are about to make, resizing the moment you connect it. If it runs past the end of the file, the rest is real silence or a loop, whichever you chose, and the node says so. Works with any model.
- **NEW: H3 Audio Sync Pixaroma.** Makes a **MiniMax H3** video perform your recording instead of the meaningless sound the model invents on its own. H3 creates the picture and the sound together as one thing, so you cannot just mute what it made and lay your own song on top: the mouth was never moving to your song. This puts your recording into the sound half and holds it there, leaving the model only the picture to decide. It reads the clip length itself so you never type a duration, hands back your track cut to exactly that length for the save node, and warns you before the render if the clip is longer than the roughly 15 seconds H3 was trained for.
- **Fixed: Image Resize Pixaroma no longer loses a see-through background.** Feed it a picture whose background has been removed and the cut-out used to come out on solid black, with no way to get the transparency back. It now comes out of the **mask** output, cropped and resized to match: wire that and the image into **Join Image with Alpha** and you have your cut-out at the new size, ready to save as a transparent PNG. You do not need to wire anything into the mask input; if you do, yours is still used. The image output stays an ordinary picture on purpose, because a picture carrying a see-through channel cannot be sent to a sampler.
...
- **Also in the LoRA panel:** a picture you set could disappear again if the panel was still loading, or if you had just used the Civitai button, because a slower answer arrived afterwards and overwrote it. Your own trigger words could go missing from the panel in the same situation. Both are fixed, and a picture dropped just outside the small box no longer lands on your canvas as a Load Image node.
