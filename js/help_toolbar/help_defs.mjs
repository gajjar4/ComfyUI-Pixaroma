  ...
  "PixaromaSaveImage": {
    ...
    sections: [
      ...
      },
      {
        ...
        defs: [
          ["%input%", "The wired name input, e.g. the filename from Load Image Pixaroma, so results keep the original name. Folders in that text are normally turned into underscores; switch on `Wired name can make folders` in the settings and they become real folders instead, so the folders you loaded from are rebuilt inside your save folder. Pair it with `Keep folder structure in the name` on Load Images from Folder."],
      ...
      },
      {
        ...
        bullets: [
          ...
          "The `gear` next to the fold triangle opens the settings (right-clicking the node still works too): date style (the order the + Date chip inserts, e.g. dd-MM-yyyy), counter digits (how many zeros %counter% uses), JPG / WebP quality, WebP lossless, workflow embedding, Civitai generation info, and whether a wired name may make folders. `Reset node size` in the right-click menu returns the node to its default size.",
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "`Keep folder structure in the name`, next to it, hands over the real path (portraits/ana) instead. Switch on `Wired name can make folders` in Save Image Pixaroma's settings as well and your whole folder tree is rebuilt in the save folder, so processing a folder of folders gives you back the same arrangement.",
