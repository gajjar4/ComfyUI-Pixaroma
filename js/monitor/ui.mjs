...
export function injectCSS() {
  if (_cssDone || document.getElementById("pix-pm-css")) {
    _cssDone = true;
    return;
  }
  _cssDone = true;
  ...
  s.textContent = [
    // ── the face ──────────────────────────────────────────────────────────
    ".pix-pm-root{width:100%;height:100%;box-sizing:border-box;user-select:none;-webkit-user-select:none;}",
    `.pix-pm-screen{width:100%;height:100%;box-sizing:border-box;background:#0c0c0e;border:1px solid #1d1d20;border-radius:${px(8)};padding:${px(M.padY)} ${px(M.padX)};display:flex;flex-direction:column;gap:${px(M.gap)};font-family:ui-monospace,"Cascadia Mono",Consolas,monospace;color:#e0e0e0;overflow:hidden;}`,

    // the small card line at the top
    `.pix-pm-title{display:flex;align-items:center;gap:${px(5)};height:${px(M.titleH)};font-size:${px(M.titleFont)};letter-spacing:.06em;color:#6b6b72;text-transform:uppercase;white-space:nowrap;overflow:hidden;flex:none;}`,
    `.pix-pm-title .dot{width:${px(5)};height:${px(5)};border-radius:50%;background:#3ec371;flex:none;}`,
    ".pix-pm-title.is-run .dot{background:" + ACC + ";}",
    ".pix-pm-title.is-off .dot{background:#5a5a60;}",
    ".pix-pm-title .name{overflow:hidden;text-overflow:ellipsis;}",
    ...
    `.pix-pm-row .pk{position:absolute;top:-1px;bottom:-1px;width:${px(2)};background:#ffd9cd;box-shadow:0 0 ${px(5)} ${ACC};display:none;}`,
    `.pix-pm-row .vl{width:${px(M.valueW)};flex:0 0 auto;text-align:right;font-variant-numeric:tabular-nums;white-space:nowrap;}`,
    ".pix-pm-row .vl small{color:#6b6b72;font-size:.86em;}",

    // the temp / power / peak line
    `.pix-pm-strip{display:flex;gap:${px(9)};height:${px(M.stripH)};align-items:center;font-size:${px(M.stripFont)};color:#8a8a8a;flex:none;overflow:hidden;white-space:nowrap;}`,
    ".pix-pm-strip b{color:#e0e0e0;font-weight:400;font-variant-numeric:tabular-nums;}",
    ".pix-pm-strip b.hot{color:#e8a33d;}",

    // the one-line strip layout
    `.pix-pm-strip1{display:flex;align-items:center;gap:${px(7)};font-size:${px(M.font)};color:#8a8a8a;flex:1;min-height:0;overflow:hidden;white-space:nowrap;}`,
    ".pix-pm-strip1 b{color:#e0e0e0;font-weight:400;font-variant-numeric:tabular-nums;}",
    ".pix-pm-strip1 .sep{color:#3a3a3a;}",
    `.pix-pm-strip1 .mini{width:${px(30)};height:${px(5)};border-radius:${px(2)};background:rgba(255,255,255,.06);position:relative;overflow:hidden;flex:none;}`,
    `.pix-pm-strip1 .mini i{position:absolute;left:0;top:0;bottom:0;width:0;background:${ACC};border-radius:${px(2)};}`,

    // buttons
    `.pix-pm-acts{display:flex;gap:${px(5)};flex-wrap:wrap;flex:none;}`,
    ...
    `.pix-pm-empty{font-size:${px(M.font)};color:#6b6b72;text-align:center;padding:${px(4)} 0;}`,
    ...
    ".lg-node:has(.pix-pm-root){background:transparent!important;border:none!important;box-shadow:none!important;}",
    // the min-height floor is node.size[1] + 30 regardless of title_mode, which
    // reserved a grey gap under the face
    ".lg-node:has(.pix-pm-root),.lg-node:has(.pix-pm-root) > div,.lg-node:has(.pix-pm-root) > div > div{min-width:0!important;min-height:0!important;}",
    ".lg-node:has(.pix-pm-root) .lg-node-header{display:none!important;}",
    ...
    ".lg-node:has(.pix-pm-root) .lg-node-widgets{grid-template-columns:minmax(0,1fr)!important;padding:0!important;row-gap:0!important;gap:0!important;}",
    ".lg-node:has(.pix-pm-root) .lg-node-widget{gap:0!important;width:100%!important;padding:0!important;}",
    ...
    ".lg-node:has(.pix-pm-root) .lg-node-widgets,.lg-node:has(.pix-pm-root) .lg-node-widgets *{pointer-events:none!important;}",
    ".lg-node:has(.pix-pm-root) .pix-pm-btn{pointer-events:auto!important;}",
    ...
    // hosts (#1a1a1a / #444).
    ".pix-pm-panel{position:fixed;z-index:10010;width:310px;max-width:94vw;background:#1a1a1a;border:1px solid #444;border-radius:6px;box-shadow:0 8px 24px rgba(0,0,0,0.6);font-family:'Segoe UI',system-ui,sans-serif;overflow:hidden;}",
    ".pix-pm-phead{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid #333;color:#ddd;font-size:13px;font-weight:600;cursor:move;}",
    ".pix-pm-px{border:0;background:transparent;color:#999;font-size:13px;cursor:pointer;padding:2px 7px;border-radius:4px;}",
    ".pix-pm-px:hover{color:#fff;}",
    ".pix-pm-pbody{max-height:74vh;overflow-y:auto;padding:4px 0;}",
    ".pix-pm-sect{padding:10px 12px;border-bottom:1px solid #2c2c2c;}",
    ".pix-pm-sect:last-child{border-bottom:0;}",
    ".pix-pm-plab{font-size:9.5px;letter-spacing:.09em;text-transform:uppercase;color:" + ACC + ";font-weight:700;margin-bottom:7px;}",
    ".pix-pm-psub{font-size:11px;line-height:1.4;color:#8a8a8a;margin:-3px 0 8px;}",
    ".pix-pm-bgrid{display:flex;flex-wrap:wrap;gap:4px;}",
    ".pix-pm-bchip{font:inherit;font-size:10.5px;padding:4px 9px;border-radius:4px;border:1px solid #444;background:#1d1d1d;color:#aaa;cursor:pointer;}",
    ".pix-pm-bchip:hover{border-color:" + ACC + ";color:#ddd;}",
    ".pix-pm-bchip.on{background:" + ACC + ";border-color:" + ACC + ";color:#fff;}",
    ".pix-pm-bchip:disabled{opacity:.4;cursor:default;}",
    ".pix-pm-bchip:disabled:hover{border-color:#444;color:#aaa;}",
    ".pix-pm-prow{display:flex;align-items:center;gap:10px;margin-top:8px;}",
    ".pix-pm-prow:first-child{margin-top:0;}",
    ".pix-pm-ptxt{flex:1;min-width:0;}",
    ".pix-pm-ptxt .t{font-size:12.5px;color:#cfcfcf;}",
    ".pix-pm-ptxt .s{font-size:11px;line-height:1.35;color:#8a8a8a;margin-top:2px;}",
    ".pix-pm-sw{width:32px;height:17px;border-radius:9px;background:#3a3a3a;position:relative;cursor:pointer;flex:none;transition:background .15s;}",
    ".pix-pm-sw i{position:absolute;top:2px;left:2px;width:13px;height:13px;border-radius:50%;background:#bbb;transition:left .15s,background .15s;}",
    ".pix-pm-sw.on{background:" + ACC + ";}",
    ".pix-pm-sw.on i{left:17px;background:#fff;}",
    ".pix-pm-qsl{-webkit-appearance:none;appearance:none;flex:1;min-width:0;height:4px;border-radius:2px;outline:none;cursor:pointer;background:linear-gradient(to right," + ACC + " var(--fill,50%),#3a3a3a var(--fill,50%));}",
    ".pix-pm-qsl::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:14px;height:14px;border-radius:50%;background:" + ACC + ";border:2px solid #1a1a1a;cursor:pointer;}",
...
function buildFace(screen, st, rows, scal, btns, hasTitle, onButton) {
  ...
  if (st.layout === "strip") {
    els.strip1 = el("div", "pix-pm-strip1");
    screen.appendChild(els.strip1);
  } else {
    for (const r of rows) {
      const row = el("div", "pix-pm-row");
      row.title = r.hint || "";
    ...
    if (!rows.length) {
      screen.appendChild(el("div", "pix-pm-empty", "Nothing to show yet"));
    }
    if (scal.length) {
      const strip = el("div", "pix-pm-strip");
      for (const s of scal) {
        const seg = el("span");
        seg.title = s.hint || "";
