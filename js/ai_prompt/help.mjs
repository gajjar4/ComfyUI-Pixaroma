  ...
  sections: [
    ...
    },
    {
      ...
      defs: [
        ...
        ["Video - prompt from a video (Gemma 4)", "Watches a clip and writes the video prompt that would make one like it: the medium, what HAPPENS from beginning to end, and what the camera does. Wire Load Video Pixaroma into the video input. Frames are read as 24fps and sampled one per second, so load a few seconds rather than a whole clip."],
